...
class GameOverSubstate extends MusicBeatSubstate {
	...
	public static var deathSoundName:String = 'fnflosssfx';
	...
	override function create() {
		...
		crack = new BGSprite('crack', 0, 0, 1, 1, ['crack'], false);
		crack.updateHitbox();
		crack.antialiasing = ClientPrefs.data.antialiasing;
		crack.alpha = 1;
		crack.animation.play('crack', true, false);
		crack.scale.set(1, 1);
		...
		retry.alpha = 0;
		retry.animation.play('idle', true, false);
		retry.scale.set(1, 1);
		retry.screenCenter(X);
		FlxTween.tween(retry, {y: retry.y + 35}, 1.5, { type: FlxTween.PINGPONG, ease: FlxEase.sineInOut});
		add(retry);
	...
	function coolStartDeath(?volume:Float = 1):Void {
		...
		FlxTween.tween(crack, {alpha: 0.75}, 1.2);
	...
	function endBullshit():Void {
		...
		FlxTween.tween(retry, {angle: -20}, 3, {ease: FlxEase.sineIn});
		FlxTween.tween(retry, {y: retry.y + 500}, 4, {ease: FlxEase.sineIn});
